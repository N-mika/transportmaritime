...
export const initializeCargoTypes = async () => {
  try {
    ...
    if (count === 0) {
      ...
      console.log(" Type de marchandise déjà présent en base, aucune insertion effectuée");
    }
  } catch (error) {
    console.error(" Erreur lors de l’initialisation des types de marchandise :", error);
  }
};